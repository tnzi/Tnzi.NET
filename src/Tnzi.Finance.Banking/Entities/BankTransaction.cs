...
public class BankTransaction : MultiTenantAuditedEntity<Guid>
{
    ...
    /// <summary>命中规则标识（匹配引擎的规则名：exact-ref / amount-date）</summary>
    public string? MatchRule { get; set; }

    /// <summary>
    /// 命中的银行规则（<see cref="BankRule"/>）——账上没有对手方时，这条流水
    /// "是什么"的解释来自哪条规则。
    /// </summary>
    /// <remarks>
    /// 只存规则 Id 而不把规则的动作复制一份到流水上：规则被改了以后，建议应当
    /// 跟着改——规则才是意图的所在。**刻意不建外键**，规则是可以被删掉的运维配置，
    /// 而流水是账本证据，不该被配置的生命周期牵着走（悬空 Id 由呈现端当作"无建议"）。
    /// </remarks>
