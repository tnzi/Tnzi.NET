...
Generate a structured JSON script file with naming pattern: `{descriptive-name}-script.json`
...
python scripts/generate.py \
  --script-file /path/to/script-file.json \
  --output-file /path/to/outputs/generated-podcast.mp3 \
  --transcript-file /path/to/outputs/generated-podcast-transcript.md
```
...
```json
{
  ...
  "locale": "en",
  "lines": [
    {"speaker": "male", "paragraph": "Hello! Welcome back to another episode."},
...
- Start with the male host saying a greeting
...
Step 1: Create script file `ai-history-script.json`:
```json
{
  "title": "The History of Artificial Intelligence",
  "locale": "en",
  "lines": [
    {"speaker": "male", "paragraph": "Hello! Welcome back to another fascinating episode. Today we're diving into something that's literally shaping our future - the history of artificial intelligence."},
...
```bash
python scripts/generate.py \
  --script-file /path/to/workspace/ai-history-script.json \
  --output-file /path/to/outputs/ai-history-podcast.mp3 \
  --transcript-file /path/to/outputs/ai-history-transcript.md
```
...
- Share both the podcast MP3 and transcript MD with the user
