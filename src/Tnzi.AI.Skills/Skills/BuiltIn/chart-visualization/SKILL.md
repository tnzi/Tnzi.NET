...
### 3. Chart Generation
Use the built-in `scripts/generate.js` to render the chart image.
...
**Execution:**
```bash
node /mnt/skills/chart-visualization/scripts/generate.js '<payload_json>'
```

The script outputs the chart image URL. If `generate.js` is not available or fails, write a custom script using the reference specs.

### 4. Result Return
Return to the user:
- The generated chart image
- The data specification used for generation
