...
public class EmailAttribute : ValidationAttribute
{
    ...
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
        ...
        if (Required && value == null)
        {
            ...
            if (localizer != null)
            {
                var localized = localizer["Validation.Email.Required", validationContext.DisplayName];
        ...
        if (localizer != null)
        {
            var localized = localizer["Validation.Email.Format", validationContext.DisplayName];
            formatErrorMessage = localized.ResourceNotFound
                ? $"{validationContext.DisplayName} must be a valid email."
                : localized.Value;
        }
        else
        {
            formatErrorMessage = $"{validationContext.DisplayName} must be a valid email.";
        }
