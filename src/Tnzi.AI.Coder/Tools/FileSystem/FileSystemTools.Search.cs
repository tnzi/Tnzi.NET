...
public partial class FileSystemTools
{
        ...
        [AIParameter("respect_gitignore", "Exclude files matching .gitignore patterns", false)] bool respectGitignore = true)
    {
        try
        {
            ...
            // 加载 .gitignore 规则 — 与 grep/repo_outline 统一使用共享 IgnorePatternMatcher，
            // 正确处理根锚定（"/foo"）、目录组件、取反等模式
            var ignoreMatcher = respectGitignore
                ? IgnorePatternMatcher.LoadFromDirectory(resolvedDir, _options.ProjectRoot)
                : null;

            // 简单 glob 实现：将 glob 转换为 searchPattern
            var files = EnumerateFilesWithGlob(resolvedDir, pattern)
                .Where(f => ignoreMatcher is null
                    || !ignoreMatcher.IsIgnored(Path.GetRelativePath(resolvedDir, f)))
