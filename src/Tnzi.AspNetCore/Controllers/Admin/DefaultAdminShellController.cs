...
public class DefaultAdminShellController : ApiAdminControllerBase
{
    ...
    /// </summary>
    [HttpGet("modules")]
    public virtual ApiResult<AdminShellModulesDto> GetModules(
        [FromServices] ITnziApplication tnziApp,
        [FromServices] IRealtimeHubRegistry? hubRegistry = null)
    {
        ...
        return Ok(new AdminShellModulesDto
        {
            Modules = modules,
            Realtime = BuildRealtime(hubRegistry),
        });
    }

    /// <summary>
    /// Report the hubs the host actually mapped, with <c>PathBase</c> applied so
    /// the client can connect to the returned path verbatim - a deployment
    /// behind an IIS sub-application therefore needs no hand-configured hub URL.
    /// A host without <c>SignalRModule</c> mapped nothing, and the client is
    /// told realtime is unavailable instead of retrying a route that will 404
    /// forever.
    /// </summary>
    private AdminShellRealtimeDto BuildRealtime(IRealtimeHubRegistry? hubRegistry)
    {
        var pathBase = Request?.PathBase.Value?.TrimEnd('/') ?? string.Empty;
        var hubs = (hubRegistry?.Hubs ?? [])
            .Select(h => new AdminShellHubDto { Name = h.Name, Path = $"{pathBase}{h.Path}" })
            .ToList();

        return new AdminShellRealtimeDto { Available = hubs.Count > 0, Hubs = hubs };
    }
