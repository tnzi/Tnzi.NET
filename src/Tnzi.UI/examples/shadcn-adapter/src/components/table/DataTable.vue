...
interface IDataTableEmits<T = unknown> {
...
const getRowKey = (row: T, index: number): string => {
  if (typeof props.rowKey === 'function') {
    ...
    if (key == null) {
      console.warn(`[DataTable] rowKey function returned ${key} for row at index ${index}, falling back to index.`);
      return String(index);
    }
    return key;
  }
  ...
  if (typeof props.rowKey === 'string') {
    console.warn(`[DataTable] rowKey "${props.rowKey}" is undefined/null for row at index ${index}, falling back to index.`);
  }
  return String(index);
};
  ...
  <div class="w-full rounded-xl border bg-card text-card-foreground shadow-sm" :class="props.class" :style="props.style">
    ...
    >
      ...
      <div class="flex gap-2">
        <Button v-if="props.exportable" variant="default" size="sm" @click="emit('export', props.exportable?.format || 'json')">
          {{ t('table.export') }}
        </Button>
        <Button v-if="props.printable" variant="default" size="sm" @click="emit('print')">
          {{ t('table.print') }}
        </Button>
      </div>
    </div>
            ...
            <td v-if="props.actions" class="px-3 py-3 text-right" @click.stop>
              <div class="inline-flex gap-2">
                <Button
                  ...
                  :key="button.key"
                  size="sm"
                  :variant="button.type === 'danger' ? 'destructive' : button.type === 'primary' ? 'primary' : 'default'"
