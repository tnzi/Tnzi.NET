...
import type { IWebDataListEmits, IWebDataListProps, IWebPagerConfig } from '@tnzi/core/components';
import { calculateTotalPages, clampPageIndex, updatePageQuery } from '@tnzi/core/utils';
import { Button } from '../primitive/ui';

const props = withDefaults(defineProps<IWebDataListProps<T>>(), {
  items: () => [] as T[],
  query: () => ({}),
  loadState: () => ({}),
  mode: 'list',
...
<template>
  <section class="space-y-3 rounded-xl border bg-card p-4 text-card-foreground" :class="props.class" :style="props.style">
    <div class="flex items-center justify-between">
      ...
      <Button variant="outline" size="sm" :disabled="isLoading" @click="onRefresh">
        {{ t('table.refresh') }}
      </Button>
    </div>
