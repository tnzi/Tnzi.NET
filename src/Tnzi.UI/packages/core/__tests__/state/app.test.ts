...
import { AppStateManager, createInitialAppState } from '../../src/state/app';
...
describe('AppStateManager', () => {
  ...
  describe('theme', () => {
    ...
    it('toggleTheme should cycle through light, dark, auto', () => {
      expect(app.theme).toBe('light');
      app.toggleTheme();
      expect(app.theme).toBe('dark');
      app.toggleTheme();
  ...
  describe('persistence', () => {
    ...
    it("loadPersistedState should migrate the legacy 'system' theme to 'auto'", () => {
      deps.storage.set('tnzi:app:theme', 'system');
      app.loadPersistedState();
      expect(app.theme).toBe('auto');
    });

    it('loadPersistedState should ignore a junk persisted theme', () => {
      app.setTheme('dark');
      deps.storage.set('tnzi:app:theme', 'chartreuse');
      app.loadPersistedState();
      // Falls back to the in-memory value rather than escaping the union.
      expect(app.theme).toBe('dark');
    });
