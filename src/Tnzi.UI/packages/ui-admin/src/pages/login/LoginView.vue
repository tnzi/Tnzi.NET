...
onMounted(async () => {
  ...
  // Routed through the identity bridge (not `useAuthApi` directly) so the page
  // has no runtime edge into `@tnzi/core/services/*` and page-level tests can
  // intercept it with `vi.mock('.../identity-bridge')`. The bridge's
  // `getAuthConfig()` already fails open to `null` on an unreachable/old
  // backend, which is exactly the "keep the merged defaults" behaviour here.
  try {
    const authConfig = await createIdentityBridge({ client }).getAuthConfig()
    if (!authConfig) return
    Object.assign(features, mergeFeatures(mapAuthConfig(authConfig), config.features))
    if (!hasConsumerThirdParty.value) {
      autoThirdParty.value = buildOAuthProviders(authConfig.oAuthProviders ?? [], client)
    }
  } catch {
