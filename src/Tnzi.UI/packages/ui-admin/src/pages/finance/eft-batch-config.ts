...
export function buildEftBatchColumns(t: (key: string) => string): ColumnDef<EftBatchRow>[] {
  return [
    ...
    { key: 'format', title: t('columns.format'), width: 100, render: (r) => (r.format ? t(EFT_FORMAT_LABEL[String(r.format)] ?? '') : '—') },
    { key: 'currency', title: t('columns.currency'), width: 90, mobileHidden: true, render: (r) => r.currency ?? '—' },
    { key: 'effectiveDate', title: t('columns.effectiveDate'), width: 120, render: (r) => formatDateOnly(r.effectiveDate, { utc: true }) },
    { key: 'totalCount', title: t('columns.count'), width: 90, render: (r) => String(r.totalCount ?? 0) },
    { key: 'totalAmount', title: t('columns.amount'), width: 140, render: (r) => amountCell(fmtMoney(r.totalAmount, r.currency)) },
    {
      key: 'status',
...
export function buildEftLineColumns(t: (key: string) => string): DataTableColumns<EftBatchLineDto> {
  return [
    { key: 'paymentNumber', title: t('lines.payment'), width: 130, render: (r) => r.paymentNumber ?? '—' },
    { key: 'payeeName', title: t('lines.payee'), minWidth: 160, render: (r) => r.payeeName ?? '—' },
    { key: 'partyBankAccountMasked', title: t('lines.bankAccount'), width: 130, render: (r) => maskedCell(r.partyBankAccountMasked) },
    { key: 'amount', title: t('lines.amount'), width: 140, render: (r) => amountCell(fmtMoney(r.amount)) },
  ]
}
