...
import { createFinanceBridge, FinanceDocumentStatus, type BillDto, type CreateBillDto } from '../../services/bridges/finance-bridge'
...
const rowActions: RowAction<FinanceDocRow>[] = [
  ...
  { key: 'post', label: 'actions.post', type: 'primary', show: isDraft, confirm: 'confirmPost', onClick: (row) => void run(() => bridge.bills.post(String(row.id ?? '')), 'postSuccess') },
  { key: 'void', label: 'actions.void', type: 'warning', show: (row) => isVoidable(row) && (row.appliedTotal ?? 0) === 0, confirm: 'confirmVoid', onClick: (row) => void run(() => bridge.bills.voidDoc(String(row.id ?? '')), 'voidSuccess') },
  { key: 'delete', label: 'actions.delete', type: 'error', show: isDraft, confirm: 'confirmDelete', onClick: (row) => void run(() => bridge.bills.deleteDraft(String(row.id ?? '')), 'deleteSuccess') },
]
</script>
