...
import type {
  ...
  UserCenterProfileField,
  UserCenterReadonlyField,
...
export interface UserCenterContext {
  ...
  /**
   * Whether a field is locked. For identity-core fields (`'email'` / `'phone'`)
   * this means "no self-service rebinding" and is deliberately independent of
   * {@link UserCenterCapabilities} - the deployment may well authenticate over
   * email while still refusing to let the user change the address themselves.
   * The two are ANDed at the call site, never substituted for one another.
   */
  isFieldReadonly: (field: UserCenterReadonlyField) => boolean
}
...
export function createUserCenterState(deps: UserCenterStateDeps): UserCenterContext {
  ...
  const readonlyFields = new Set<UserCenterReadonlyField>(deps.config.profile?.readonlyFields ?? [])
