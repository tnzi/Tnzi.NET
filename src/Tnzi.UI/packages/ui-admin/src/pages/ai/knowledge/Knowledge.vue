<template>
  <!--
    Knowledge — Phase 5 Task 5.10. Standard CRUD over knowledge bases plus
    a per-row Reindex action surfaced inside the edit form.

    Follows the Agents canonical pattern (Task 5.2). Reindex deviation:
  ...
  >
    <template #form="{ formData, mode }">
      <TFormSchemaRenderer
        ...
        :translate="t"
      />
      <div v-if="mode === 'edit' && (formData as Record<string, unknown>)?.id" class="knowledge-reindex">
        <button
          type="button"
          class="knowledge-reindex__btn"
          :disabled="reindexBusy"
          @click="onReindex((formData as Record<string, unknown>).id as string)"
        >
          {{ reindexBusy ? 'Reindexing…' : 'Reindex' }}
        </button>
        <span v-if="reindexStatus" class="knowledge-reindex__status" :data-status="reindexStatus.kind">
...
import { knowledgeColumns, knowledgeFormSchema } from './knowledge-config'
...
const crud = useCrudPage<KnowledgeRow>({
  pageId: 'ai.knowledge',
  columns: knowledgeColumns,
  rowKey: (row) => String((row as { id?: string }).id ?? ''),
  fetchData: (query) => bridge.knowledge.fetch(query),
  createData: (data) => bridge.knowledge.create(data as KnowledgeRow),
  updateData: (id, data) => bridge.knowledge.update(String(id), data as KnowledgeRow),
  deleteData: (ids) => bridge.knowledge.delete(ids.map(String)),
})

const rowActions: RowAction<KnowledgeRow>[] = [editAction(crud), deleteAction(crud)]
...
.knowledge-reindex__btn {
  padding: 0.4rem 0.9rem;
  cursor: pointer;
}
.knowledge-reindex__btn:disabled {
  cursor: wait;
  opacity: 0.6;
}
.knowledge-reindex__status[data-status='error'] {
