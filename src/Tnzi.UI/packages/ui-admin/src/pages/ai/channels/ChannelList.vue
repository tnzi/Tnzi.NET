<template>
  <!--
    ChannelList — surfaces /admin/channels/* and /admin/gateway/* exposed by
    `Tnzi.AI.Channels`. Two tabs:
      ...
      • Gateway  — IGateway control-plane (WebSocket connections + session
                   binding rules)
  -->
  <div class="t-channels-page t-stack-page">
    <div class="t-channels-page__header">
      <div class="t-channels-page__title">
        <TSvgIcon icon="mdi:connection" :size="20" />
        <h2>{{ t('title') }}</h2>
      </div>
      <div class="t-channels-page__toolbar">
...
const connectionColumns: DataTableColumns<GatewayConnectionInfo> = [
  {
    ...
    width: 220,
    render: (row) =>
      h('code', { style: 'font-family: var(--tnzi-font-mono); font-size: 11px;' }, row.connectionId),
  },
...
const bindingColumns: DataTableColumns<SessionBindingRuleDto> = [
  ...
  },
  {
    ...
    render: (r) => h('code', { style: 'font-family: var(--tnzi-font-mono); font-size: 11px;' }, r.agentId),
  },
]
...
.t-channels-page__title {
  display: flex;
  align-items: center;
  gap: 8px;
}
.t-channels-page__title h2 {
  margin: 0;
  font-size: 18px;
  font-weight: 600;
}
.t-channels-page__toolbar {
  display: flex;
  align-items: center;
  gap: 8px;
}
