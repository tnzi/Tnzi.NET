<template>
  <!--
    McpServerList — Phase 5 Task 5.11. Entity-driven CRUD over EXTERNAL MCP
    ...
    Follows the ProviderConfig canonical pattern (Task 5.8) — same
  ...
  >
    <template #form="{ formData, mode }">
      ...
      <div
        ...
        class="mcp-server-list__test"
      >
        <button
          type="button"
          class="mcp-server-list__btn"
          :disabled="testBusy"
...
import { mcpServerColumns, mcpServerFormSchema } from './mcp-server-list-config'
import type {
...
.mcp-server-list__btn {
  padding: 0.4rem 0.9rem;
  cursor: pointer;
}
.mcp-server-list__btn:disabled {
  cursor: wait;
  opacity: 0.6;
}
