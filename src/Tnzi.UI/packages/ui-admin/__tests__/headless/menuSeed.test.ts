...
describe('exportRouteMenuSeed', () => {
  it('flattens a menu tree into seed rows keyed by menuKey (route name)', () => {
    const menus: AdminMenuItem[] = [
      ...
      { key: 'workbench', label: 'Workbench', path: '/workbench', meta: { title: 'Workbench', order: 0 } },
    ]
    const seed = exportRouteMenuSeed(menus)
    // flattened: identity + 2 children + workbench = 4
    ...
    expect(byKey['workbench'].name).toBe('Workbench')
    ...
    expect(byKey['workbench'].type).toBe(MenuType.Menu)
  })
