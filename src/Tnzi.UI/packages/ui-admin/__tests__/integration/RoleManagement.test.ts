...
vi.mock('../../src/services/bridges/identity-bridge', () => ({
  createIdentityBridge: () => ({
    ...
    roles: {
      fetch: vi.fn(async () => ({
        items: [
          { id: 'r1', name: 'admin', code: 'ADMIN', enabled: true, description: 'Full access' },
          { id: 'r2', name: 'viewer', code: 'VIEWER', enabled: true, description: 'Read-only' },
        ],
        totalCount: 2,
        pageIndex: 1,
        pageSize: 20,
      })),
      create: vi.fn(async (data: unknown) => ({ ...(data as object), id: 'r3' })),
...
describe('RoleManagement (Phase 3 page)', () => {
  beforeEach(() => { setActivePinia(createPinia()) })

  it('mounts and fetches roles on load', async () => {
    const wrapper = mount(RoleManagement, { global: { stubs } })
    await nextTick()
