...
/**
 * Items page — TCrudPage with account/tax-code field renderers.
 */
...
vi.mock('vue-router', () => ({
  useRoute: () => ({ query: {}, params: {}, path: '/admin/finance/items', fullPath: '/admin/finance/items', hash: '', name: 'finance.items', meta: {} }),
  useRouter: () => ({ push: vi.fn(), replace: vi.fn(), back: vi.fn() }),
}))

const fetchList = vi.fn(async () => ({ items: [{ id: 'i1', name: 'Consulting', type: 'Service', isActive: true }], totalCount: 1, pageIndex: 1, pageSize: 20 }))
...
import Page from '../../../src/pages/finance/Items.vue'
