...
describe('ChatSession page (Phase 3.29)', () => {
  beforeEach(() => { setActivePinia(createPinia()) })

  it('mounts without throwing', async () => {
    const { default: ChatSession } = await import('../../src/pages/chat/ChatSession.vue')
    const wrapper = mount(ChatSession, { global: { stubs } })
    await nextTick()
...
describe('chatSessionColumns config', () => {
  it('exports columns with required keys', async () => {
    const { chatSessionColumns } = await import('../../src/pages/chat/chat-session-config')
  ...
  it('exports formSchema with required fields', async () => {
    const { chatSessionFormSchema } = await import('../../src/pages/chat/chat-session-config')
