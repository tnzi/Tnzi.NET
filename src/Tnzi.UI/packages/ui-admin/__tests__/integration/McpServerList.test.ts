...
/**
 * Phase 5 Task 5.11 — McpServerList integration test.
 * Mirrors KbManager.test.ts (CRUD + extra action canonical pattern).
 */
...
describe('McpServerList page (Phase 5 Task 5.11)', () => {
  beforeEach(() => {
  ...
  it('mounts, fetches MCP server registrations on mount, and displays rows', async () => {
    const wrapper = mount(McpServerList, { global: { stubs } })
    await flushPromises()
