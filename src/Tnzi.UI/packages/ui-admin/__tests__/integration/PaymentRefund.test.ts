...
describe('PaymentRefund page (Phase 3.34)', () => {
  beforeEach(() => { setActivePinia(createPinia()) })

  it('mounts without throwing', async () => {
    const { default: PaymentRefund } = await import('../../src/pages/payment/PaymentRefund.vue')
    const wrapper = mount(PaymentRefund, { global: { stubs } })
    await nextTick()
...
describe('refundColumns config', () => {
  it('exports columns with required keys', async () => {
    const { refundColumns } = await import('../../src/pages/payment/payment-refund-config')
  ...
  it('exports formSchema with required fields', async () => {
    const { refundFormSchema } = await import('../../src/pages/payment/payment-refund-config')
