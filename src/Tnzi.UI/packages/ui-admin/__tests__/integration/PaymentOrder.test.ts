...
describe('PaymentOrder page (Phase 3.32)', () => {
  beforeEach(() => { setActivePinia(createPinia()) })

  it('mounts without throwing', async () => {
    const { default: PaymentOrder } = await import('../../src/pages/payment/PaymentOrder.vue')
    const wrapper = mount(PaymentOrder, { global: { stubs } })
    await nextTick()
  ...
  it('renders statistics banner with stats cards', async () => {
    ...
    expect(wrapper.find('.payment-order-stats').exists()).toBe(true)
  })
})

describe('orderColumns config', () => {
  it('exports columns with required keys', async () => {
    const { orderColumns } = await import('../../src/pages/payment/payment-order-config')
  ...
  it('exports formSchema with required fields', async () => {
    const { orderFormSchema } = await import('../../src/pages/payment/payment-order-config')
