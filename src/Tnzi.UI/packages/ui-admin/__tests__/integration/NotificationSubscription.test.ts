...
describe('NotificationSubscription page (Phase 3.27)', () => {
  beforeEach(() => { setActivePinia(createPinia()) })

  it('mounts without throwing', async () => {
    const { default: NotificationSubscription } = await import('../../src/pages/notification/NotificationSubscription.vue')
    const wrapper = mount(NotificationSubscription, { global: { stubs } })
    await nextTick()
...
describe('notificationSubscriptionColumns config', () => {
  it('exports columns with required keys aligned to NotificationPreferenceDto', async () => {
    const { notificationSubscriptionColumns } = await import('../../src/pages/notification/notification-subscription-config')
  ...
  it('exports formSchema with required fields', async () => {
    const { notificationSubscriptionFormSchema } = await import('../../src/pages/notification/notification-subscription-config')
