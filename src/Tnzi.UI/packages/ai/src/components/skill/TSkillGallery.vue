  ...
  <div class="space-y-3">
    ...
    <ScrollArea class="max-h-[500px]">
      ...
      <div v-else class="grid grid-cols-2 gap-2">
        <TSkillCard v-for="skill in filtered" :key="skill.id" :skill="skill" @activate="emit('activate', $event)" @deactivate="emit('deactivate', $event)" @click="emit('skill-click', $event)" />
      </div>
    </ScrollArea>
  </div>
</template>
