...
describe('TEmpty', () => {
  ...
  /**
   * The prop name the earlier `@tnzi/ui` version of this component used. Kept
   * working so moving the admin implementation down here did not silently blank
   * anything that had been passing `description`.
   */
  it('accepts the legacy `description` prop', () => {
    const w = mount(TEmpty, { props: { description: 'Nothing here' } })
    expect(w.find('.t-empty__text').text()).toBe('Nothing here')
  })

  it('prefers `text` when both are given', () => {
    const w = mount(TEmpty, { props: { text: 'A', description: 'B' } })
    expect(w.find('.t-empty__text').text()).toBe('A')
  })

  it('supports the icon and description slots', () => {
    const w = mount(TEmpty, {
      slots: { icon: '<i class="custom-icon" />', description: '<em class="custom-desc">nope</em>' },
    })
    expect(w.find('.custom-icon').exists()).toBe(true)
    expect(w.find('.custom-desc').exists()).toBe(true)
    expect(w.findComponent({ name: 'TSvgIcon' }).exists()).toBe(false)
  })
})
