<template>
  <div class="section-page">
      ...
      <PreviewBox>
        <div style="display: flex; flex-direction: column; gap: 24px;">
          <div style="display: flex; align-items: center; gap: 16px;">
            <div v-for="s in ['xs', 'sm', 'md', 'lg', 'xl', '2xl', '3xl']" :key="s" style="text-align: center;">
              <TIcon icon="lucide:home" :size="s" />
              <div class="size-label">{{ s }}</div>
            </div>
          </div>
          <div style="display: flex; align-items: center; gap: 16px;">
            <TIcon icon="lucide:loader-2" size="lg" spin />
            <TIcon icon="material-symbols:favorite" size="lg" color="#d03050" />
            <TIcon icon="carbon:search" size="lg" />
            <TIcon icon="lucide:settings" :size="32" />
          </div>
        </div>
      </PreviewBox>
    </div>

    <!-- UnoCSS Pure CSS Icons -->
    <div class="demo-block">
      <h2>UnoCSS Pure CSS Icons</h2>
      <p class="demo-desc">Static icons rendered as pure CSS (zero JS runtime) via UnoCSS <code>preset-icons</code>. Use <code>class="i-{collection}:{name}"</code>.</p>
      <PreviewBox>
        <div style="display: flex; align-items: center; gap: 20px;">
          <div class="i-lucide:home text-2xl" />
          <div class="i-lucide:settings text-2xl" />
          <div class="i-material-symbols:favorite text-2xl text-red-500" />
          <div class="i-carbon:search text-2xl" />
          <div class="i-lucide:bell text-2xl text-amber-500" />
          <div class="i-material-symbols:check-circle text-2xl text-green-500" />
        </div>
      </PreviewBox>
    </div>
    ...
    <div class="demo-block">
      ...
      <p class="demo-desc">Layout and spacing utilities with Naive UI CSS variable shortcuts.</p>
      <PreviewBox :no-badge="true">
        <div style="width: 100%; padding: 16px; display: flex; flex-direction: column; gap: 16px;">
          <div class="flex-between p-4 border-naive rounded-lg">
            <span class="text-naive-primary font-medium">Left Title</span>
            <span class="text-naive-secondary text-sm">Right Description</span>
          </div>
          <div class="flex items-center gap-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            <div class="i-lucide:info text-lg text-blue-500" />
            <span class="text-sm">Info message styled with UnoCSS utilities</span>
          </div>
          <div class="grid grid-cols-3 gap-3">
            <div class="flex-col-center p-4 border-naive rounded-lg gap-2">
              <div class="i-lucide:users text-xl text-indigo-500" />
              <span class="text-xs text-naive-secondary">Users</span>
            </div>
            <div class="flex-col-center p-4 border-naive rounded-lg gap-2">
              <div class="i-lucide:shopping-cart text-xl text-emerald-500" />
              <span class="text-xs text-naive-secondary">Orders</span>
            </div>
            <div class="flex-col-center p-4 border-naive rounded-lg gap-2">
              <div class="i-lucide:bar-chart-2 text-xl text-amber-500" />
              <span class="text-xs text-naive-secondary">Analytics</span>
            </div>
          </div>
        </div>
      </PreviewBox>
