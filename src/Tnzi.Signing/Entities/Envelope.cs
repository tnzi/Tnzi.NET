...
/// </para>
/// <para>
/// ★ <b>带乐观并发戳的理由是防重复密封。</b>并行签署（<see cref="IsSequential"/> = false）时，
/// 最后两位可能同时提交：收件人查询走 <c>AsNoTracking</c>，两边都会从库里读到"全签完"，
/// 于是各密封一次 —— 两份成品、两个哈希，而先交给宿主归档的那一份不是最后记在这条记录上的
/// 那一份。哈希与成品对不上，这份签署的不可否认性就没了。密封前用这枚戳做一次
/// CAS 抢占，输的一方在动手之前退出（同 <c>Estimate</c> 防并发双转换生成两张发票）。
/// </para>
/// </remarks>
public class Envelope : FullAuditedEntity<Guid>, IMultiTenant, IConcurrencyStamp
{
    /// <summary>租户ID</summary>
    public Guid? TenantId { get; set; }

    /// <summary>并发标记（每次更新自动变更）。见类型说明里"防重复密封"一段。</summary>
