...
public class WorkflowNodeExecutor
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (var attempt = 0; attempt < maxAttempts; attempt++)
        {
            try
            {
                ...
                // 记录 Trace
                await RecordTraceAsync(run, stepId, "node_execute", new
                {
        ...
        await RecordTraceAsync(run, stepId, "node_error", new
        {
    ...
    private IWorkflowNode ResolveNode(WorkflowStepDto step)
    {
        ...
        var defaultAgentNode = registeredNodes.FirstOrDefault(n =>
            string.Equals(n.NodeType, "agent", StringComparison.OrdinalIgnoreCase));
