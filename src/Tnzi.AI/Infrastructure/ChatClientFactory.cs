...
public class ChatClientFactory : IChatClientFactory
{
    ...
    private volatile ConcurrentDictionary<string, IChatClient> _chatClients = new();
    private volatile ConcurrentDictionary<string, IEmbeddingGenerator<string, Embedding<float>>> _embeddingClients = new();
    private volatile ConcurrentDictionary<string, ChatClient> _openAIChatClients = new();
    private volatile ConcurrentDictionary<string, EmbeddingClient> _openAIEmbeddingClients = new();
    ...
    internal ChatClient GetOpenAIChatClientInternal(string? providerName = null, string? model = null)
    {
        ...
        return _openAIChatClients.GetOrAdd(cacheKey, _ =>
        {
            var provider = ResolveProviderImpl(name);
    ...
    internal EmbeddingClient GetOpenAIEmbeddingClientInternal(string? providerName = null, string? model = null)
    {
        ...
        return _openAIEmbeddingClients.GetOrAdd(cacheKey, _ =>
        {
            var provider = ResolveProviderImpl(name);
            var nativeClient = provider.CreateNativeClient(providerOptions);
            if (nativeClient is OpenAIClient openAIClient)
