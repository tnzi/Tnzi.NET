...
public class BroadcastService : ApplicationService, IBroadcastService
{
    ...
    public async Task<Result<int>> BroadcastToUsersAsync(IEnumerable<Guid> userIds, string content)
    {
        ...
        var preview = content.Length <= 100 ? content : content[..100];
        var delivered = 0;
    ...
    public async Task<Result<int>> BroadcastToRoleAsync(Guid roleId, string content)
    {
        ...
        return await BroadcastToUsersAsync(userIds, content);
    }
